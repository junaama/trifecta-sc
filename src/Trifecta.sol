...
contract Trifecta {
     ...
     */
    constructor(address _zkVerifier, address _reputationRegistry) {
        owner = msg.sender;
    ...
    function requestLoan(
        ...
        require(
            verificationResults[_proofHash].isValid,
            "Invalid or unverified ZK proof"
        );
        require(
            !verificationResults[_proofHash].isProcessed,
            "ZK proof already used"
        );

        uint256 requiredCollateral = (offer.amount *
            offer.requiredCollateralRatio) / 10000;
        require(msg.value >= requiredCollateral, "Insufficient collateral");

        uint256 reputationScore = verificationResults[_proofHash]
            .reputationScore;
        require(
            reputationScore >= offer.minimumReputationScore,
    ...
    function makePayment(
        ...
        if (msg.value >= remaining) {
            ...
            if (loan.collateralAmount > 0) {
                (bool collateralSent, ) = loan.borrower.call{
                    value: loan.collateralAmount
                }("");
                require(
                    collateralSent,
                    "Failed to return collateral to borrower"
                );
            }
